...
import { ExamsSidebar } from "@/components/dashboard/exams/exams-sidebar";
        ...
        <div className="mx-auto flex max-w-[1500px] flex-col gap-3 px-4 py-3 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div className="flex items-center justify-between gap-3">
            <Link href="/" className="flex items-center gap-3 font-bold">
              ...
              <span>
                EduCBC Pro
                <span className="block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Exams department</span>
              </span>
            </Link>
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            <label className="flex min-h-10 w-full items-center gap-2 rounded border border-border bg-white px-3 text-sm md:w-80">
              ...
              <span className="text-muted-foreground">Search assessments, reports, learners</span>
            </label>
            <nav className="flex flex-wrap gap-2 text-sm font-semibold">
              <Link href="/dashboard/exams" className="rounded bg-primary px-3 py-2 text-primary-foreground">Exams</Link>
              <Link href="/dashboard/teacher" className="rounded px-3 py-2 hover:bg-muted">Teacher</Link>
              <Link href="/dashboard/super-admin" className="rounded px-3 py-2 hover:bg-muted">Super Admin</Link>
            </nav>
          </div>
        </div>
      </header>
