    ...
    <aside className="space-y-4">
      <div className="rounded border border-border bg-white p-4 shadow-soft">
        <div className="flex items-center justify-between gap-3">
          <Link href="/dashboard/exams" className="flex min-w-0 items-center gap-3 font-bold">
            ...
            <span className="min-w-0">
              Exams Department
              <span className="block truncate text-xs font-semibold text-muted-foreground">{examsProfile.officer}</span>
            </span>
          </Link>
          <button
            type="button"
            ...
            aria-expanded={open}
            aria-label="Toggle exams navigation"
          >
            {open ? <X className="h-4 w-4" aria-hidden="true" /> : <Menu className="h-4 w-4" aria-hidden="true" />}
          </button>
        </div>
        <div className="mt-4 grid grid-cols-2 gap-2 text-xs font-semibold">
          <span className="rounded bg-blue-50 px-2.5 py-2 text-blue-800">{examsProfile.completion} complete</span>
          <span className="rounded bg-orange-50 px-2.5 py-2 text-orange-800">{examsProfile.pendingApprovals} approvals</span>
        </div>
      </div>
      ...
      <nav className={`${open ? "block" : "hidden"} rounded border border-border bg-white p-3 shadow-soft lg:block`}>
        <Link
          ...
          className={`flex min-h-10 items-center rounded px-3 py-2 text-sm font-bold ${
            pathname === "/dashboard/exams" ? "bg-primary text-primary-foreground" : "hover:bg-muted"
          }`}
        ...
        <div className="mt-3 space-y-1">
          {examsModules.map((item) => {
            ...
            const href = `/dashboard/exams/${item.slug}`;
